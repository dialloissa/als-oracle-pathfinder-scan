...
const readDecode= fname => (new Buffer(fs.readFileSync(fname, 'ascii'), 'ascii'));
...
class Config {
    constructor() {
        ...
        this.env = env;
        // Our secret key
        this.keyPath = process.env['CLIENT_KEY_FILENAME'] || `${root}/../../../helm/als-oracle-pathfinder/secrets/clientkey.pem`;
        // The cert we'll use to identify ourselves to the pathfinder service
        this.certPath = process.env['CLIENT_CERT_FILENAME'] || `${root}/../../../helm/als-oracle-pathfinder/secrets/clientcert.pem`;
        // Pathfinder's cert, we'll use this to authenticate pathfinder

        this.caPath = process.env['CA_CERT_FILENAME'] || `${root}/../../../helm/als-oracle-pathfinder/secrets/pathfindercert.pem`;
        if (env === 'live') {
            this.caPath = process.env['CA_CERT_FILENAME'] || `${root}/../../../helm/als-oracle-pathfinder/secrets/pathfindercert_root_cert.pem`;
            this.caPathIntermediate = process.env['CA_CERT_INTERMEDIATE_FILENAME'] || `${root}/../../../helm/als-oracle-pathfinder/secrets/pathfindercert_intermediate_cert.pem`;
        }
        ...
        try {
            ...
            throw new Error(`Error occurred loading TLS files- you may need to obtain these from pathfinder: ${err}`);
        }
    }
}
